...
def make_dict(train_path):
    ...
    for line in text:
        ...
        word_list = word_list.union(set(line))#集合取并集
    ...
    #add the <pad>空白 and <unk_word>未出现过的单词，未知
    word2number_dict["<pad>"] = 0
    number2word_dict[0] = "<pad>"
    word2number_dict["<unk_word>"] = 1
    number2word_dict[1] = "<unk_word>"
    word2number_dict["<sos>"] = 2
    number2word_dict[2] = "<sos>"#开头
    word2number_dict["<eos>"] = 3
    number2word_dict[3] = "<eos>"#结尾
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        #i_t
        self.W_ii = nn.Parameter(torch.Tensor(emb_size,n_hidden))
        ...
        self.b_g = nn.Parameter(torch.Tensor(n_hidden))
        ...
        #i_t_up
        self.W_ii_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.W_hi_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.b_i_up = nn.Parameter(torch.Tensor(n_hidden))

        #f_t_up
        self.W_if_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.W_hf_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.b_f_up = nn.Parameter(torch.Tensor(n_hidden))

        #g_t_up
        self.W_ig_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.W_hg_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.b_g_up = nn.Parameter(torch.Tensor(n_hidden))

        #o_t_up
        self.W_io_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.W_ho_up = nn.Parameter(torch.Tensor(n_hidden,n_hidden))
        self.b_o_up = nn.Parameter(torch.Tensor(n_hidden))  
    ...
    def forward(self, X):
        ...
        for t in range(n_step):
            ...
            #第二层
            i_t_up=torch.sigmoid(h_t@self.W_ii_up+h_t_up@self.W_hi_up+self.b_i_up)
            f_t_up=torch.sigmoid(h_t@self.W_if_up+h_t_up@self.W_hf_up+self.b_f_up)
            g_t_up=torch.tanh(h_t@self.W_ig_up+h_t_up@self.W_hg_up+self.b_g_up)
            o_t_up=torch.sigmoid(h_t@self.W_io_up+h_t_up@self.W_ho_up+self.b_o_up)
            c_t_up=f_t_up*c_t_up+i_t_up*g_t_up
            h_t_up=o_t_up*torch.tanh(c_t_up)
        
        model=self.W(h_t_up)+self.b
        return model
