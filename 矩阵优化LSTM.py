...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.W_i = nn.Parameter(torch.Tensor(emb_size,n_hidden*4))
        self.W_h = nn.Parameter(torch.Tensor(n_hidden,n_hidden*4))
        self.bb = nn.Parameter(torch.Tensor(n_hidden*4))      
    ...
    def forward(self, X):
        ...
        for t in range(n_step):
            ...
            MM = x_t@self.W_i+h_t@self.W_h+self.bb
            i_t,f_t,g_t,o_t=(
                torch.sigmoid(MM[:,:part]),
                torch.sigmoid(MM[:,part:part*2]),
                torch.tanh(MM[:,part*2:part*3]),
                torch.sigmoid(MM[:,part*3:part*4]),
                )
            c_t = f_t*c_t+i_t*g_t
